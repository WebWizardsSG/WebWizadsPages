...
function createWhiteBall() {
  ...
  let whiteballs = Matter.Bodies.circle(whiteX, whiteY, ballDiameter / 2, {
    ...
    restitution: 0.95, // Adjust restitution
    friction: 0.02, // Adjust friction
...
function displayClickedBall() {
  if (whiteBall[0]) {
    if (initialPlacement || placingCueBall) {
      ...
      stroke(0);
      textSize(15);
      text("Set position of cue ball, double click", mouseX - 80, mouseY - 40);
    } else {
      if (
        ...
        stroke(0);
        textSize(15);
        text("Ready to launch", mouseX - 80, mouseY - 40);
      }
    }
  }
}
